...
def generate_tex_report():
    ...
    # Format Table 1 LaTeX Rows
    t1_rows = []
    for _, row in df_t1.iterrows():
        t1_rows.append(
            f"{row['region']} & {row['mean_gdp']:,.2f} & {row['share']:.2f}\\% & {row['growth']:.2f}\\% & {row['volatility']:.2f} \\\\"
        )
    t1_table_body = "\n".join(t1_rows)
    ...
    # Format Table 3 LaTeX Rows
    t3_rows = []
    for _, row in df_t3.iterrows():
        t3_rows.append(
            f"{int(row['Year'])} & {row['Gini Coefficient']:.4f} & {row['Theil Index']:.4f} & {row['HHI (Output Concentration)']:.4f} \\\\"
        )
    t3_table_body = "\n".join(t3_rows)
