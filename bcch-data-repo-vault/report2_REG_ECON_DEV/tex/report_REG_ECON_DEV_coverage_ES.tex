...
El capítulo \textit{Regionales} del catálogo del Banco Central de Chile contiene \textbf{2306 series de tiempo únicas}. Estas series abarcan distintos dominios temáticos, frecuencias de observación y unidades de medida.

> \textbf{Sobre este número.} El capítulo tiene \textbf{3881 filas} en el catálogo, pero sólo \textbf{2306 códigos distintos}: el catálogo repite un mismo código bajo varios nombres de cuadro. El código `F035.PIB.FLU.R.CLP.2018.Z.Z.Z.15.0.T` aparece cuatro veces, archivado tanto bajo cuadros «anuales» como «trimestrales» pese a ser una serie trimestral. Contar filas cuenta entradas de cuadro, no series. Todas las cifras de este reporte se calculan sobre códigos únicos.


\subsection{Reconciliación con el universo del pipeline}

El pipeline no selecciona series por capítulo sino por \textbf{parseabilidad de región}: si el código resuelve a una de las 16 regiones, entra. Las dos cifras miden cosas distintas y ninguna contiene a la otra.
...
La última fila es la importante: *\textit{1785 series regionales viven fuera del capítulo }Regionales**\textit{, la mayoría en }Cuentas Nacionales* —que es precisamente donde reside el PIB regional por actividad de la familia F035. Un inventario construido sobre el capítulo las omitiría por completo. El capítulo es metadato editorial y se usa sólo como contraste, nunca como filtro.
...
\begin{itemize}
  \item \textbf{Anual (A)}: \textbf{1061 series}. Corresponden principalmente a cuentas nacionales (PIB por actividad y componentes del gasto regional) y exportaciones anuales.
  \item \textbf{Trimestral (T)}: \textbf{458 series}. Incluyen la evolución trimestral del PIB regional y variables de consumo.
  \item \textbf{Mensual (M)}: \textbf{753 series}. Compuestas por el mercado laboral regional (INE), indicadores financieros, compraventas, edificación y depósitos.
...
\begin{itemize}
  \item \textbf{Cuentas Nacionales (PIB y Consumo)}: 882 series
  \item \textbf{Finanzas y Sistema Financiero Regional}: 345 series
  \item \textbf{Otros Indicadores Regionales}: 289 series
  \item \textbf{Exportaciones Regionales}: 214 series
  \item \textbf{Transacciones y Ventas Locales}: 210 series
  \item \textbf{Desarrollo Territorial y Uso de Suelo}: 171 series
  \item \textbf{Indicadores Sectoriales Corto Plazo}: 125 series
  \item \textbf{Mercado Laboral}: 70 series
...
\begin{enumerate}
  ...
  \item \textbf{Finanzas y Sistema Financiero Regional}: Con \textbf{345 series}, cuenta con un monitoreo de frecuencia mensual, registrando saldos y números de cuentas de ahorro y crédito.
  \item \textbf{Desarrollo Territorial y Uso de Suelo}: Con \textbf{171 series}, se actualiza mensualmente (INE), recopilando variables físicas críticas como metros cuadrados autorizados para edificación y parque vehicular.
...
\section{Cobertura Sectorial ($s = 1 \dots 13$)}
...
\subsection{Los 13 Sectores Económicos Analizados}
\begin{enumerate}
...
\begin{itemize}
  \item \textbf{Homogeneidad Sectorial del PIB}: Las 16 regiones tienen series sectoriales asignadas para cada uno de los 13 sectores bajo la clasificación del PIB. Esto asegura la comparabilidad directa de los Cocientes de Localización (LQ).
